...
# Pretrained model
model = models.resnet50(pretrained=ResNet50_Weights.IMAGENET1K_V2)  # vgg11_bn(pretrained=True)
...
# unfreeze all layers to fine-tune for the new task
for param in model.parameters():
    param.requires_grad = True
...
with open(f"{results_dir}/LPFT_{num_epochs}epochs_{int(time.time())}.txt", "w") as f:
